...
class DATA_MEMBERS{
  protected: 
    ...
    int dis_x,dis_y;
    bool save,chk;
    int textIndex=-1,spaceChar=0;
    vector<char> redo,undo;
            ...
            case '\b' : save=false;
                        ...
                        if(text!=""){ //while string is not empty,delete the char
                          ...
                          if(temp!=' '){
                            undo.pop_back();
                            spaceChar=0;
                          }else if(spaceChar==1){ //continue space
                            undo.pop_back();
                          }else
                            spaceChar=1; //first space

                            undo.push_back(temp);//push character in undo stack after backspace
                            undo.push_back(textIndex);//push index of that char
                            textIndex--;
